...
const finalizeClientConnection = (client, socket) =>
  new Promise((resolve, reject) => {
    let timer_connect = undefined
    socket.setEncoding('utf8')
    socket.on('error', reject)
    if (client._config.connection_timeout !== undefined) {
      timer_connect = setTimeout(() => onTimeout(), client._config.connection_timeout)
    }
